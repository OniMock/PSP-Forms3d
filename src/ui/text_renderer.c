...
// Simple 8x8 bitmap font (subset: ASCII 32-126)
static const uint8_t font8x8_basic[95][8] = {
    ...
    {0xFC, 0x66, 0x66, 0x7C, 0x6E, 0x66, 0xE6, 0x00}, // R
    ...
    {0xC0, 0xC0, 0xC6, 0xD8, 0xF0, 0xD8, 0xC6, 0x00}, // k
  ...
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      if (glyph[row] & (1 << (7 - col))) {
        platform_draw_pixel(x + col, y + row, color);
      }
    }
  }
}
...
void text_draw_string_right(int x, int y, const char *str, Color color) {
  int len = strlen(str);
  text_draw_string(x - (len * 8), y, str, color);
}

void text_draw_string_center(int x, int y, const char *str, Color color) {
  int len = strlen(str);
  text_draw_string(x - (len * 4), y, str, color);
}
